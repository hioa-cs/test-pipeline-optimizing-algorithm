...
def get_cost(filename):
    ...
    testcosts = dict((rows[0],rows[1]) for rows in dataset)
    print "Collecting test and costs"
#    print testcosts
    return testcosts
...
##################################### REORDER ALGORITHM #######################################
#Using Lists
def reorder(tests,pre_tests,sub_tests,no_of_swap,testcosts):
...
#                    print "  -> rx_sub_test_minpos: " + str(rx_sub_test_minpos) + " ry_pre_test_maxpos: " + str(ry_pre_test_maxpos)
                    ...
                    if int(xpos) > int(ry_pre_test_maxpos) and int(ypos) < int(rx_sub_test_minpos):
                        ...
                        if float(y_prob_fail) > float(x_prob_fail) :
                            ...
                            cost_factor = 1 - abs(float(testcosts[txname]) - float(testcosts[tyname]))
                            print "cost factor: ", cost_factor

                            cost_value = delta_new * cost_factor
...
#                            print "   -> delta is " + str(delta_new)
                            # print delta_new
                            cost_factor = 1 - abs(float(testcosts[tyname]) - float(testcosts[txname]))
                            print "cost factor: ", cost_factor
...
def main():
    ...
    testcosts = get_cost(costfilename)
    ...
    delta_max, swapcounter, time_now = reorder(tests, pre_tests, sub_tests, no_of_swap, testcosts)
    ...
    outfile_name = '{0}{1}{2}'.format(filename,"_cost_sorted",".csv")
